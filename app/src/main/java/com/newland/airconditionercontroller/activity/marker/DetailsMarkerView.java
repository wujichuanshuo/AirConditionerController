...
public class DetailsMarkerView extends MarkerView {
    ...
    public void refreshContent(Entry e, Highlight highlight) {
        ...
        try {
            ...
            if (e.getY() == 0) {
                ...
                mTvChart1.setText(concat(e.getY(), "分钟："));
            }
            mTvMonth.setText(String.valueOf((int) e.getX() + 1).concat("分前"));
        } catch (Exception e1) {
            e1.printStackTrace();
        }
        super.refreshContent(e, highlight);
    ...
    public String concat(float money, String values) {
        return values + new BigDecimal(money).setScale(2, BigDecimal.ROUND_HALF_UP).toPlainString() + "人";
    }
